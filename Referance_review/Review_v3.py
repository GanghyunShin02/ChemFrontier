...
RH_in=Constant(domain,default_scalar_type(0.5))
...
P1_T=basix.ufl.element("Lagrange",domain.basix_cell(),1) # temperature
...
V_C,VC_to_W=W.sub(2).collapse()  # 산소농도용..
V_W,VW_top_W=W.sub(3).collapse() # 물농도용....
#V_T,VT_to_W=W.sub(4).collapse()  # 온도용..
...
u_in = Function(V)
u_in.interpolate(lambda x: np.vstack((np.full(x.shape[1], U_IN), np.zeros(x.shape[1]))))
...
w.sub(0).interpolate(lambda x: np.vstack((np.full(x.shape[1], U_IN), np.zeros(x.shape[1]))),cells0=channeltag)
...
def Pv_sat_numeric(T_kelvin):
    T_c = T_kelvin - 273.15
    return 101325.0 * 10.0**(-2.1794 + 0.02953*T_c - 9.1837e-5*T_c**2 + 1.4454e-7*T_c**3)

def CW_in_from_RH(RH, T_kelvin, Pc_val, MH2O_val=0.018, Mair_val=0.029):
    P_v = RH * Pv_sat_numeric(T_kelvin)
    return (P_v*MH2O_val) / (P_v*MH2O_val + (Pc_val - P_v)*Mair_val)



def inlet_composition_from_RH(RH, T_kelvin, Pc_val,
                               X_O2_dry=0.21, MO2_val=0.032,
                               MN2_val=0.028, MH2O_val=0.018):
    P_v = RH * Pv_sat_numeric(T_kelvin)
    X_H2O = min(P_v / Pc_val, 0.98)
    dry_scale = 1.0 - X_H2O
    X_O2 = X_O2_dry * dry_scale
    X_N2 = (1.0 - X_O2_dry) * dry_scale
    M_mix = X_O2*MO2_val + X_N2*MN2_val + X_H2O*MH2O_val
    Y_O2  = X_O2 * MO2_val / M_mix
    Y_H2O = X_H2O * MH2O_val / M_mix
    return Y_O2, Y_H2O
...
Cg_H2O = ufl.min_value(CW, Cg_sat)

rv_ratio  = (Pv_sat*MH2O) / (Pc*Mair)          # 식(21)
gamma_H2O = lambda_l + lambda_g*Cg_H2O       # 식(24)
...
DO2=darcy_switch*((epsilon*(1-s))**tau)*DgO2+(1-darcy_switch)*DgO2
...
D_eff_w = rho_mix*(epsilon*(1 - s)**tau)*DgH2O + D_cap*ds_dCw
#water_diff_flux = rho_mix * D_eff_w * grad(CW)

Dw_gdl = (epsilon*(1 - s))**tau * DgH2O + D_cap*ds_dCw
Dw_ch  = DgH2O

water_diff_flux = rho_mix * (darcy_switch*Dw_gdl + (1-darcy_switch)*Dw_ch) * grad(CW)
...
FO2 = (
    ...
    + rhoO2*gamma_O2 * C * u_n_out * vC * ds_measure(16)
...
# 시각화용 별도 Function들 (mixed space에서 뽑아서 개별 파일로 저장)
C_viz  = Function(V_C)
C_viz.name = "O2_mass_fraction"

CW_viz = Function(V_W)
CW_viz.name = "H2O_mass_fraction"

s_viz  = Function(V_C)      # s_expr_compiled이 V_C 기반이라 같은 space 사용
s_viz.name = "saturation"

vtx_C  = VTXWriter(domain.comm, "/home/ss/ChemFrontier/viz_O2.bp",  [C_viz],  engine="BP4")
vtx_CW = VTXWriter(domain.comm, "/home/ss/ChemFrontier/viz_H2O.bp", [CW_viz], engine="BP4")
vtx_s  = VTXWriter(domain.comm, "/home/ss/ChemFrontier/viz_s.bp",   [s_viz],  engine="BP4")
...
def reset_state(eta_start=0.05):
    w.x.array[:] = 0.0
    w.sub(0).interpolate(lambda x: np.vstack((np.full(x.shape[1], U_IN), np.zeros(x.shape[1]))),
                          cells0=channeltag)
    w.sub(2).interpolate(lambda x: np.full(x.shape[1], 0.21))

    CW_in_val = CW_in_from_RH(float(RH_in.value), float(T.value), float(Pc.value))
    CW_in.interpolate(lambda x: np.full(x.shape[1], CW_in_val))
    w.sub(3).interpolate(lambda x: np.full(x.shape[1], CW_in_val))


    Y_O2_in, Y_H2O_in = inlet_composition_from_RH(
        float(RH_in.value), float(T.value), float(Pc.value))

    CO2_in.interpolate(lambda x: np.full(x.shape[1], Y_O2_in))
    w.sub(2).interpolate(lambda x: np.full(x.shape[1], Y_O2_in))

    CW_in.interpolate(lambda x: np.full(x.shape[1], Y_H2O_in))
    w.sub(3).interpolate(lambda x: np.full(x.shape[1], Y_H2O_in))
...
def run_eta_sweep(eta_max=1.5, d_eta_init=0.01, max_bisect=4):
    ...
    hist = {"eta": [], "I": [], "s_max": [], "V": [],"C_min":[],"CW_max":[]}
    ...
    while float(eta.value) < eta_max:
        ...
        hist["CW_max"].append(float(CW_sol.x.array.max()))

            # --- 시각화 필드 갱신 및 기록 ---
        C_viz.interpolate(w.sub(2).collapse())
        CW_viz.interpolate(w.sub(3).collapse())
        s_viz.x.array[:] = s_proj.x.array   # 이미 계산해둔 s_proj 재사용

        step_time = float(eta.value)   # ParaView 타임스텝으로 eta 값을 그대로 사용
        vtx_C.write(step_time)
        vtx_CW.write(step_time)
        vtx_s.write(step_time)
...
for RH_v in RH_vals:
    RH_in.value = RH_v
    reset_state(eta_start=0.05)   # 여기서 CW_in이 RH_v 기준으로 재계산됨
    if MPI.COMM_WORLD.rank == 0:
        print(f"[RH={RH_v}]", flush=True)
    results_RH[RH_v] = run_eta_sweep(eta_max=1.5)


if MPI.COMM_WORLD.rank == 0:
    plt.figure()
    for RH_v, hist in results_RH.items():
        I_cm2 = np.array(hist["I"]) * 1e-4
        plt.plot(I_cm2, hist["eta"], marker='o', label=f"RH={RH_v}")
    plt.xlabel("I (A/cm$^2$)")
    plt.ylabel("eta (V)")
    plt.title(f"Effect of inlet RH (T={T_fixed}K, Pc={Pc_fixed:.3e}Pa)")
    plt.legend()
    plt.show()
    ...
    for RH_v, hist in results_RH.items():
        I_cm2 = np.array(hist["I"]) * 1e-4
        plt.plot(I_cm2, hist["s_max"], marker='o', label=f"RH={RH_v}")
    plt.xlabel("I (A/cm$^2$)")
    plt.ylabel("s_max")
    plt.title(f"Effect of inlet RH on s_max")
    plt.legend()
    plt.show()
    ...
    for RH_v, hist in results_RH.items():
        I_cm2 = np.array(hist["I"]) * 1e-4
        plt.plot(I_cm2, hist["C_min"], marker='o', label=f"RH={RH_v}")
    plt.xlabel("I (A/cm$^2$)")
    plt.ylabel("Oxygen C_min")
    plt.title(f"Effect of inlet RH on Oxygen C_min")
    plt.legend()
    plt.show()
    ...
    for RH_v, hist in results_RH.items():
        I_cm2 = np.array(hist["I"]) * 1e-4
        plt.plot(I_cm2, hist["V"], marker='o', label=f"RH={RH_v}")
    plt.xlabel("I (A/cm$^2$)")
    plt.ylabel("voltage")
    plt.title(f"Voltage and Current density vs RH")
    plt.legend()
    plt.show()
    ...
    for RH_v, hist in results_RH.items():
        I_cm2 = np.array(hist["I"]) * 1e-4
        plt.plot(I_cm2, hist["CW_max"], marker='o', label=f"RH={RH_v}")
    plt.xlabel("I (A/cm$^2$)")
    plt.ylabel("CW")
    plt.title(f"Water concentrate and Current density vs RH")
    plt.legend()
    plt.show()
