...
OUT = "/home/ss/ChemFrontier/ML/pemfc_results"
os.makedirs(OUT, exist_ok=True)
LOG_PATH = f"{OUT}/higheta_tau1.1_Lx0.2mm_progress.log"
...
def build_problem(overrides):
    ...
    problem = NonlinearProblem(F_total, w, bcs=bcs, petsc_options=snes_petsc_options,
                                petsc_options_prefix=f"pemfc_higheta_{id(overrides)}_",
                                form_compiler_options={"quadrature_degree": 4})

    # Bound-constrained Newton (SNES VI): confirmed the hard way that an
    # ordinary unconstrained Newton solve can converge to a state with
    # CgO2 (or Cw) slightly negative -- physically meaningless, and at
    # high eta the Tafel exponential (~1e11 by eta=1.5) turns even a tiny
    # negative CgO2 into a wildly unphysical (negative) current. Rather
    # than detecting this after solving, constrain the SOLUTION VECTOR
    # itself to stay within [0,1] on the Cw/CgO2 sub-spaces at every
    # Newton iterate -- PETSc's reduced-space active-set VI solver
    # (vinewtonrsls) is designed exactly for this: same nonlinear system,
    # solved subject to box constraints on selected components.
    problem.solver.setType("vinewtonrsls")
    Cw_dofs_global = W.sub(1).collapse()[1]
    CgO2_dofs_global = W.sub(2).collapse()[1]
    # Build the bound arrays entirely in numpy first, then assign the
    # raw buffer directly via .array[:] -- confirmed the hard way that
    # Vec.setValues(indices, values) has a fussier call signature than
    # expected here (size-mismatch error even with matching-length
    # arrays); direct buffer assignment sidesteps that API entirely.
    n_total = len(w.x.array)
    lb_arr = np.full(n_total, -1e20)  # p_hat (and everything else by default): unbounded
    ...
    def check_physical(I_now, tol=0.01, I_tol=-50.0):
        """Confirmed the hard way TWICE that this is necessary: (1) Newton
        can 'converge' to a state where CgO2 (or Cw) has drifted slightly
        negative even with box constraints active (the VI solver keeps the
        RAW dof values in bounds, but doesn't touch what happens to
        derived/projected quantities); (2) even with (1) fixed, the
        AVERAGE current I itself can still come out substantially negative
        (-0.67 A/cm^2 observed) near a sharp flooding front, from L2-
        projection overshoot in computing that average -- a real, separate
        failure mode from the raw-concentration one, confirmed to slip
        straight past a Cw/CgO2-only check. I_tol (in A/m^2, i.e. -50 =
        -0.005 A/cm^2) allows tiny numerical noise near I=0 without
        flagging genuinely fine low-current solutions, while still
        catching anything resembling the -6731 A/m^2 seen in practice."""
        ...
        I_ok = I_now > I_tol
        ok = conc_ok and I_ok
        return ok, dict(Cw_min=Cw_min, Cw_max=Cw_max, CgO2_min=CgO2_min, CgO2_max=CgO2_max,
                         I=I_now)

    return w, eta_const, problem, measure, check_physical


def adaptive_polarization_curve(overrides, eta_target, ds_init=0.02, ds_min=5e-4, log_fn=alog,
                                 checkpoint_path=None, s_max_flooding_thresh=0.05,
                                 ds_flooding_cap=0.005, s_max_jump_thresh=0.1,
                                 big_jump_multipliers=(1.0, 2.0, 3.0)):
    """Genuine restore-on-failure + step halving; stops gracefully (not
    indefinitely) once ds shrinks below ds_min -- interpreted as at/near
    this setting's own fold (or some other kind of obstruction -- this
    notebook doesn't try to classify it, just reports where it happens).
    ...
    I0, s0 = measure()
    results = [(eta, I0, s0, problem.solver.getIterationNumber())]
    w_last_good = w.x.array.copy()
    s_last_accepted = s0
    ds = ds_init
    consecutive_failures = 0  # tracks repeated failure AT THE SAME eta -- confirmed the
                               # hard way that plain halving after every failure means a
                               # genuinely hard spot costs one expensive Newton failure PER
                               # halving (each 10-20+ min near a real obstruction) before ds
                               # finally shrinks enough to reveal it; escalating the shrink
                               # once we already know a point is difficult gets to that
                               # answer in far fewer expensive attempts.

    def write_checkpoint():
        if checkpoint_path is None:
            return
        eta_c = np.array([r[0] for r in results])
        I_c = np.array([r[1] for r in results])
        s_c = np.array([r[2] for r in results])
        it_c = np.array([r[3] for r in results])
        np.savez(checkpoint_path, eta=eta_c, I_avg=I_c, s_max=s_c, iters=it_c, complete=False)
    ...
    while eta < eta_target - 1e-9:
        ...
        jump_rejected = newton_ok and phys_ok and s_jump is not None and s_jump > s_max_jump_thresh
        # If Newton converged AND stayed physically valid, and the ONLY
        # problem is the jump-size policy, and ds is already about to
        # bottom out anyway (no more room to resolve this more finely) --
        # just accept it. Confirmed the hard way that the alternative
        # (shrinking to ds_min and giving up, or jumping far ahead with a
        # BIG JUMP) throws away resolution we'd already legitimately
        # earned: Newton itself was never the obstacle here, only our own
        # bookkeeping choice was.
        at_floor = (ds / 2.0) < ds_min
        force_accept = jump_rejected and at_floor
        if newton_ok and phys_ok and (not jump_rejected or force_accept):
            ...
            accept_note = (f" [accepted despite s_max jump of {s_jump:.4f} -- ds was already "
                            f"at its floor, so shrinking further wasn't going to help]"
                            if force_accept else "")
            ...
            # First failure at this point: halve as before (still gives a
            # rough sense of where trouble starts). Second+ consecutive
            # failure at the SAME point: shrink hard (8x) -- we already
            # know this spot is difficult, no need to keep re-confirming
            # that one halving at a time.
            ...
            if not newton_ok:
                ...
                reason = (f"REJECTED: s_max jumped by {s_jump:.4f} in one step "
                          f"(from {s_last_accepted:.4f} to {s_now:.4f}), exceeding "
                          f"s_max_jump_thresh={s_max_jump_thresh}")
            ...
            if ds < ds_min:
                # Before giving up: try one or more BIG JUMPS from here,
                # deliberately BYPASSING the s_max-jump rejection (only
                # that check -- Newton convergence and physical validity
                # are still required). Confirmed the hard way that a
                # steep transition like this one can be IMPOSSIBLE to
                # cross via small steps (Newton gets stuck on
                # intermediate states that don't connect smoothly) while
                # a single large jump lands cleanly on the far side --
                # the earlier, purely-incremental version of this stepper
                # would give up right at the base of such a wall even
                # though a well-defined state exists just past it.
                ...
                for jump_mult in big_jump_multipliers:
                    ...
                    if newton_ok_big and phys_ok_big:
                        ...
                        results.append((eta_try_big, I_big, s_big,
                        ...
                        w.x.array[:] = w_last_good
                        w.x.scatter_forward()
                        reason_big = "Newton FAILED" if not newton_ok_big else f"REJECTED ({phys_info_big})"
                        log_fn(f"  BIG JUMP to eta={eta_try_big:.4f} (x{jump_mult:.1f} ds_init) "
                                f"also failed ({reason_big})")
                if jumped:
                    continue
                log_fn(f"  ds below {ds_min} and all BIG JUMP fallbacks failed -- stopping here "
                        f"(possible fold, physical-bounds breakdown, or other obstruction near "
                        f"eta={eta:.4f} -- NOT further classified by this exploratory notebook)")
                break
